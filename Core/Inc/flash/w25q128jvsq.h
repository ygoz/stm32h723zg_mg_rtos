 ...
 * @brief Header file for W25Q128JV QSPI Flash driver for STM32.
 * 
 * This file defines constants, command macros, and the driver interface for
 * the W25Q128JV flash memory chip using Octo/Quad SPI (OSPI/QSPI) on STM32.
 * It includes register access macros, memory layout definitions, and high-level
 * APIs for initialization, read, write, erase, and memory-mapped mode support.
 * 
 * The driver provides:
 * - Flash initialization and configuration (quad mode, reset, etc.)
 * - Sector, block, and chip erase
 * - Byte/page writing
 * - Memory-mapped access
 * - Status register read/write
 * 
 * This file also includes unit test utilities for runtime verification of
 * flash functionality via `w25q128_test()`.
 * 
 * @version 0.1
 * @date 2025-07-29
 * 
 * @note Dependencies: STM32 HAL, Mongoose, CMSIS
 */
 ...
 *   - reset_chip             : Resets the flash chip.
 *   - read_status_registers  : Reads the specified status register.
 *   - write_status_registers : Writes to the specified status register.
 *   - test                   : Performs a functional test of the flash (erase, write, verify).
 *
 * Example:
 * @code
...
typedef struct {
    ...
    HAL_StatusTypeDef (*reset_chip)(void);
    HAL_StatusTypeDef (*read_status_registers)(uint8_t *register_data, uint8_t register_num);
    HAL_StatusTypeDef (*write_status_registers)(uint8_t reg_data, uint8_t reg_num);
    HAL_StatusTypeDef (*test)(void);
} qspi_flash_driver_t;
