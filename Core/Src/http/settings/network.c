...
uint8_t set_network_settings(network_settings *data, struct mg_mgr *mgr) {
    ...
    uint8_t status = flash_write_data(NETWORK_SETTINGS_ADDR, (uint32_t *)data, num_words);

    if (status == 0) {
        mg_timer_add(mgr, 500, 0, set_ip_configurations, mgr);  // Schedule IP config update
    }
...
// this function sets the ethernet driver the new network settings
...
MG_IRAM void set_ip_configurations(void *arg) {
	...
	MG_INFO(("DEBUG: is_initialized = %u, !value = %d\n", raw, !raw));
    ...
    // You now have both mgr and settings
		...
		MG_INFO(("setting not init, using default settings\r\n\r\n\r\n"));
