...
int Write(uint32_t Address, uint32_t Size, uint8_t* buffer)
{
    ...
    if (W25Q128_OSPI_Write(&hospi2, (uint8_t*) buffer, (Address & (0x0fffffff)), Size) != HAL_OK)
    {
    	__disable_irq(); //disable interrupts
        return LOADER_FAIL;
    }
...
int SectorErase(uint32_t EraseStartAddress, uint32_t EraseEndAddress)
{
    ...
    if (W25Q128_OSPI_EraseSector(&hospi2, EraseStartAddress, EraseEndAddress) != HAL_OK)
    {
    	__disable_irq(); //disable interrupts
        return LOADER_FAIL;
    }
...
int MassErase(void)
{
    ...
    if (W25Q128_OSPI_Erase_Chip(&hospi2) != HAL_OK)
    {
    	__disable_irq(); //disable interrupts
        return LOADER_FAIL;
    }
